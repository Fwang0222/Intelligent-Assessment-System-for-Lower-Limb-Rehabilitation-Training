...
class RealtimeTrainingPage(BasePage):
    ...
    def setup_ui(self) -> None:
        ...
        header_card, header_layout = self.create_section_card(
            "Runtime",
            "V3 pipeline uses local video source, YOLO pose detection, ROI extraction, and Qwen multimodal evaluation.",
        )
        header_layout.addWidget(BodyLabel(f"Inference runtime: {self.runtime_config.summary_text()}"))
        ...
        top_layout.addWidget(BodyLabel("Video source:"))
        ...
        self.phase_label = BodyLabel("Current phase: -")
        self.rep_label = BodyLabel("Repetitions: 0")
        self.completion_label = BodyLabel("Completion status: -")
        ...
        self.camera_status_label = BodyLabel("Source status: idle")
        self.provider_status_label = BodyLabel("Pipeline provider: -")
        self.inference_status_label = BodyLabel("Inference status: idle")
        ...
        right_widget.addWidget(BodyLabel("Risk alerts:"))
        ...
        right_widget.addWidget(BodyLabel("Set-level feedback:"))
    ...
    def start_training(self) -> None:
        ...
        self.phase_label.setText("Current phase: Start")
        self.rep_label.setText("Repetitions: 0")
        self.completion_label.setText("Completion status: In Progress")
        self.score_label.setText("Current score: collecting data...")
        self.subscore_label.setText("Subscores: -")
        ...
        self.camera_status_label.setText(
            f"Source status: {self.vision_pipeline.mode}" if source_ok else "Source status: mock stream mode"
        )
        self.provider_status_label.setText("Pipeline provider: YOLO=- | Qwen=- | Qwen interval=5s")
        self.inference_status_label.setText("Inference status: YOLO thread idle | Qwen thread idle")
        self.timer.start(80)
    ...
    def _thread_status_text(self) -> str:
        ...
        return f"Inference status: YOLO thread {yolo_state} | Qwen thread {qwen_state}"
    ...
    def _render_result(self, result: dict) -> None:
        ...
        if pipeline_meta:
            self.provider_status_label.setText(
                f"Pipeline provider: YOLO={pipeline_meta.get('yolo_provider', '-')} | "
                f"Qwen={pipeline_meta.get('qwen_provider', '-')} | "
                f"Qwen interval={pipeline_meta.get('qwen_interval_seconds', 5)}s"
            )
        self.phase_label.setText(f"Current phase: {result.get('action_phase', '-')}")
        self.rep_label.setText(
            f"Repetitions: {result.get('rep_count', 0)}  |  Set {result.get('set_index', 1)}/{self.session_manager.sets_count}"
        )
        self.completion_label.setText(f"Completion status: {result.get('completion_status', 'In Progress')}")
        ...
        self.subscore_label.setText(
            f"Subscores - Range: {result['range_score']}, Rhythm: {result['rhythm_score']}, "
        ...
        self._set_plain_text_if_changed(
            ...
            f"Current completion: {round(result.get('completion_rate', 0) * 100, 2)}%\n"
            "Tip: Keep movement smooth and avoid compensation.",
        )
    ...
    def on_timer_tick(self) -> None:
        ...
        self.camera_status_label.setText(f"Source status: {source_frame.get('source_mode', self.vision_pipeline.mode)}")
    ...
    def stop_training(self) -> None:
        ...
        self.completion_label.setText(
            f"Completion status: {'Completed' if payload.get('completion_rate', 0) >= 1.0 else 'Partially completed'}"
        )
        ...
        self.camera_status_label.setText("Source status: idle")
        self.provider_status_label.setText("Pipeline provider: idle")
        self.inference_status_label.setText("Inference status: idle")
