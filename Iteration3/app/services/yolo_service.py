...
class YoloPoseAdapter:
    ...
    def _simplify_coco_keypoints(self, points: List[Point]) -> List[Point]:
        """Convert COCO 17-point pose to the 7-point lower-limb skeleton used by the app."""
        if len(points) < 17:
            return points
        ...
        return [
            ...
            hip,            # hip center
            points[13],     # left knee
            points[15],     # left ankle
            points[14],     # right knee
            points[16],     # right ankle
        ]
    ...
    def _infer_ultralytics(self, frame_rgb, expected_action: str) -> Dict | None:
        if self.model is None:
            return None
        try:
            results = self.model(frame_rgb, verbose=False, device=self.config.yolo_device or "cpu")
            if not results:
                return None
            result = results[0]
            ...
            full_points = [(float(p[0]), float(p[1])) for p in pts_raw]
            points = self.smoother.apply(self._simplify_coco_keypoints(full_points))

            frame_h = frame_rgb.shape[0]
            frame_w = frame_rgb.shape[1]
            roi = self._roi_from_points(full_points, frame_w=frame_w, frame_h=frame_h)
