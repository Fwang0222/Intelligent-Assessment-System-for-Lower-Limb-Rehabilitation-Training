"""Main application window and page registration."""

from PySide6.QtWidgets import QApplication

from app.ui.fluent_compat import FluentWindow, get_icon
from app.ui.pages.analysis_page import AnalysisPage
from app.ui.pages.doctor_page import DoctorPage
from app.ui.pages.history_page import HistoryPage
from app.ui.pages.home_page import HomePage
from app.ui.pages.plan_page import PlanPage
from app.ui.pages.profile_page import ProfilePage
from app.ui.pages.realtime_training_page import RealtimeTrainingPage
from app.ui.pages.result_page import ResultPage


class RehabMainWindow(FluentWindow):
    def __init__(self, db, user, parent=None):
        app = QApplication.instance()
        if app is not None:
            app.setProperty("rehab_role", user.get("role", "patient"))
            app.setProperty("rehab_user_name", user.get("full_name", user.get("username", "Rehab User")))
        super().__init__(parent)
        self.db = db
        self.user = user
        self.setWindowTitle("Lower-Limb Rehabilitation Intelligent Assessment System - V5")
        self.resize(1400, 900)
        self.init_pages()

    def init_pages(self) -> None:
        self.home_page = HomePage(self.db, self.user)
        self.doctor_page = None
        self.profile_page = None
        self.realtime_page = None
        self.result_page = None
        self.plan_page = None
        self.history_page = None
        self.analysis_page = None

        if self.user.get("role") == "doctor":
            self.doctor_page = DoctorPage(self.db, self.user)
            page_defs = [
                (self.home_page, "home_page", "Dashboard", "home"),
                (self.doctor_page, "doctor_page", "Doctor Dashboard", "doctor"),
            ]
        else:
            self.profile_page = ProfilePage(self.db, self.user)
            self.realtime_page = RealtimeTrainingPage(self.db, self.user, refresh_callback=self.refresh_all_pages)
            self.result_page = ResultPage(self.db, self.user)
            self.plan_page = PlanPage(self.db, self.user, refresh_callback=self.refresh_all_pages)
            self.history_page = HistoryPage(self.db, self.user)
            self.analysis_page = AnalysisPage(self.db, self.user)
            page_defs = [
                (self.home_page, "home_page", "Dashboard", "home"),
                (self.profile_page, "profile_page", "Rehab Profile", "profile"),
                (self.realtime_page, "realtime_page", "Real-Time Training", "realtime"),
                (self.result_page, "result_page", "Training Results", "result"),
                (self.plan_page, "plan_page", "Training Plans", "plan"),
                (self.history_page, "history_page", "History Records", "history"),
                (self.analysis_page, "analysis_page", "Data Analysis", "analysis"),
            ]

        for page, name, title, icon_key in page_defs:
            page.setObjectName(name)
            self.addSubInterface(page, get_icon(icon_key), title)

    def refresh_all_pages(self) -> None:
        for page in [
            self.home_page,
            self.profile_page,
            self.realtime_page,
            self.result_page,
            self.plan_page,
            self.history_page,
            self.analysis_page,
            self.doctor_page,
        ]:
            if hasattr(page, "refresh_data"):
                page.refresh_data()
